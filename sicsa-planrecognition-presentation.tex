...
%---------------------------------------------------------------------------------
    ...
    \begin{frame}{Example (2 of 2) - $h_{uniq}$}
...
\fi
	...
	\begin{frame}[c]\frametitle{Extracting Landmarks in Continuous Domains}
		\begin{columns}
			\begin{column}{0.5\textwidth}
				\begin{enumerate}
					\item<1-> Partition landmarks using wall aligned squares.
					\item<2-> Recursively divide squares with multiple goals using the mid-point in $x,y$ between two randomly selected goals.
				\end{enumerate}
				% \onslide<1->{
...
% 	\fi
	...
	\begin{frame}[c]\frametitle{Goal Mirroring with Landmarks}
		...
		\begin{columns}
			...
			\begin{itemize}
				\item Compute landmarks and optimal plans $i_k$ for all goals
				\item For every observation:\\
				...
				% 	\item Compute plan prefix, and for every goal
					\begin{itemize}
						...
						\item Compute plan suffix $m_{k}^{+}$ (from last observation) using planner
						\item Compute \textbf{cost ratio} between prefix+suffix and optimal plan: $P(O \mid g) = \frac{\textit{cost}(i_k)}{\textit{cost}(m^-) + \textit{cost}(m_k+)}$
					\end{itemize}
				% \end{itemize}
...
% 			\end{column}
		...
		\begin{itemize}
			\item Rank unpruned goals based on a \textbf{normalized cost ratio}\\
			Computes $P(g_k \mid O)$ using a normalizing factor $\eta = 1/\sum_{g_k \in G} rank(g_k)$
...
% \if\masterclass1
	\begin{frame}[c]\frametitle{Discrete Evaluation}
		\begin{itemize}
			...
			{\footnotesize \textsc{Blocks-World, Campus, Depots, Driver-Log, Dock-Worker-Robots, Easy-IPC-Grid, Ferry, Intrusion-Detection, Kitchen, Logistics, Miconic, Rovers, Satellite, Sokoban, and Zeno-Travel}}; 
...
\begin{itemize}
	...
	\item This example has $2^{5}$ completions \\(2 possible preconditions and 3 possible effects)
\end{itemize}
\end{frame}
...
\begin{frame}[c]\frametitle{Experiments}
	\begin{itemize}
		...
		\end{enumerate}
		\item Results show accuracy and runtime
	\end{itemize}
\end{frame}

\begin{frame}[c]\frametitle{Accuracy for Low Incompleteness}
	\begin{columns}
...
\begin{frame}[c]\frametitle{Accuracy for High Incompleteness}
	\begin{columns}
		\begin{column}{0.45\textwidth}
			\centering
...
\begin{frame}[c]\frametitle{Recognition Runtime}
	% \todo{Add graph from Ramon}
	\begin{center}
		\includegraphics[width=0.7\linewidth]{recognition_time_incomplete/recognition_time.pdf}
	\end{center}
\end{frame}
